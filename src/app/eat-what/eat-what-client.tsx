...
import { classifyMealInput, normalizeMealConversation, parseMentionedPriceCents, truncateMealMessage } from "@/lib/meal-input-routing";
...
export function EatWhatClient() {
  ...
  function openManualBookkeeping() {
    const recentUserTexts = [userRequest, ...[...conversation].reverse().filter((message) => message.role === "user").map((message) => message.content)];
    const detectedPriceCents = recentUserTexts.reduce<number | null>((detected, text) => detected ?? parseMentionedPriceCents(text), null);
    const draftId = crypto.randomUUID();
    setError("");
    setPurchaseMessage("");
    setPendingPurchase({
      item: {
        candidateId: `manual-${draftId}`,
        name: "",
        recommendationType: "OVERALL",
        risk: "用户从餐食页面手动发起记账",
        priceCents: detectedPriceCents ?? 1,
      },
      runId: `manual-${draftId}`,
      source: "CHAT",
      idempotencyKey: crypto.randomUUID(),
      actualPrice: detectedPriceCents === null ? "" : centsToYuan(detectedPriceCents),
      occurredAt: localDateTimeValue(),
    });
  }
  ...
  async function confirmPurchase() {
    ...
    setError("");
    try {
      if (!pendingPurchase.item.name.trim()) throw new Error("请填写餐食或项目名称");
  ...
  return <main className="app-page meal-page px-4 py-8 text-slate-900 sm:px-6 sm:py-10"><div className="relative mx-auto max-w-5xl">
    ...
    {pendingPurchase && <div className="fixed inset-0 z-70 grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="purchase-confirm-title"><section className="w-full max-w-xl rounded-[1.75rem] border border-emerald-200 bg-emerald-50 p-5 shadow-2xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-emerald-700">Bookkeeping</p><h2 id="purchase-confirm-title" className="mt-1 text-xl font-black text-emerald-950">确认实际消费</h2></div><button type="button" disabled={confirmingPurchase} onClick={() => { setPendingPurchase(null); setError(""); }} aria-label="关闭记账窗口" className="grid size-9 place-items-center rounded-xl border border-emerald-200 bg-white text-emerald-800 hover:bg-emerald-100">×</button></div><p className="mt-3 text-sm leading-6 text-emerald-900">可手动补全 Agent 没有识别出的决定；只有点击最终确认后才会写入消费记录。</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-bold text-emerald-950">餐食 / 项目<input autoFocus value={pendingPurchase.item.name} onChange={(event) => setPendingPurchase({ ...pendingPurchase, item: { ...pendingPurchase.item, name: event.target.value } })} placeholder="例如：日料套餐" className="rounded-xl border border-emerald-300 bg-white px-3 py-2.5 font-normal" /></label><label className="grid gap-1.5 text-sm font-bold text-emerald-950">实际金额（元）<input value={pendingPurchase.actualPrice} onChange={(event) => setPendingPurchase({ ...pendingPurchase, actualPrice: event.target.value })} inputMode="decimal" placeholder="例如：25.00" className="rounded-xl border border-emerald-300 bg-white px-3 py-2.5 font-normal" /></label><label className="grid gap-1.5 text-sm font-bold text-emerald-950 sm:col-span-2">消费时间<input type="datetime-local" value={pendingPurchase.occurredAt} onChange={(event) => setPendingPurchase({ ...pendingPurchase, occurredAt: event.target.value })} className="rounded-xl border border-emerald-300 bg-white px-3 py-2.5 font-normal" /></label></div>{error && <p role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}<div className="mt-5 flex flex-wrap justify-end gap-3"><button type="button" disabled={confirmingPurchase} onClick={() => { setPendingPurchase(null); setError(""); }} className="rounded-xl border border-emerald-700 px-4 py-2.5 font-semibold text-emerald-900">取消</button><button type="button" disabled={confirmingPurchase} onClick={() => void confirmPurchase()} className="rounded-xl bg-emerald-800 px-5 py-2.5 font-semibold text-white disabled:opacity-50">{confirmingPurchase ? "正在记账…" : "确认购买并记账"}</button></div></section></div>}
