  ...
  async function analyze() {
    ...
    try {
      ...
      setSelected(new Set(data.candidates
        .filter((item) => item.duplicateStatus === "NEW" && item.type !== "REFUND" && !item.needsReview)
      ...
      <section className="surface-card mt-6 overflow-hidden rounded-3xl"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-100 px-4 py-3"><div><h2 className="font-bold text-slate-900">按状态聚合的候选记录</h2><p className="mt-1 text-xs text-slate-500">需复核记录优先集中展示，确认后再统一导入。</p></div><span className="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">表格可横向滑动 →</span></div><div className="data-scroll overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm">
        ...
        <tbody>{candidateGroups.map((group) => <Fragment key={group.key}>
          ...
          {group.items.map((item) => {
            const refundBlocked = item.type === "REFUND" && !item.originalTransactionId;
            return <tr key={item.temporaryId} className="border-t border-stone-100 align-top">
              ...
              <td className="p-3"><select value={item.type} onChange={(event) => update(item.temporaryId, { type: event.target.value as ImportedTransactionCandidate["type"], category: event.target.value === "INCOME" ? null : item.category })} className="rounded-lg border px-2 py-1"><option value="EXPENSE">支出</option><option value="INCOME">收入</option><option value="REFUND">退款</option></select></td>
              ...
              <td className="p-3"><select value={item.category ?? ""} disabled={item.type === "INCOME"} onChange={(event) => update(item.temporaryId, { category: event.target.value as TransactionCategory })} className="rounded-lg border px-2 py-1"><option value="">请选择</option>{transactionCategories.map((category) => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></td>
              <td className="p-3"><div className="grid gap-1 text-xs">{item.duplicateStatus === "POSSIBLE_DUPLICATE" && <span className="font-semibold text-red-700">可能重复</span>}{item.needsReview && <span className="font-semibold text-amber-700">需复核</span>}{refundBlocked && <span className="text-red-700">退款需在退款页面关联原支出</span>}{item.reviewReasons.map((reason) => <span key={reason} className="text-slate-500">{reason}</span>)}</div></td>
            </tr>;
          })}
        </Fragment>)}</tbody>
      </table></div></section>
